   ...
   // MARK: - Load
    ...
    fileprivate lazy var localDataURL: URL? = {
        ...
        if let url = self.localDataDirectoryURL {
            let fileUrl = url.appendingPathComponent("crazyBook.json")
            return fileUrl
        }
        return nil
    }()
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
		...
		if let tabController = window?.rootViewController as? UITabBarController,
			...
			for (_, c) in viewControllers.enumerated() {
				if let navController = c as? UINavigationController {
					
					switch navController.topViewController {
						
					case let tableVCController as TableVC:
						tableVCController.recordBook = crazyBook
					case let filtertableVCController as FilterTableVC:
						filtertableVCController.recordBook = crazyBook
					case let customVCController as CustomVC:
						customVCController.recordBook = crazyBook
					default: break
					}
				}
			}
